...
TAGS = {"metadata", "hide_input", "hide_output", "prompt"}
TAGS.update(FENCES)
...
class Cell:
    ...
    def output(self):
        ...
        for row in self.cell["outputs"]:
            ...
            # Store image as file and output an image tag
                ...
                path = self.lesson_root / "episodes" / "fig" / filename
                try:
                    open(path)
            ...
            # Pretty print HTML if possible. This is just for pandas tables so far.
                ...
                pattern = r"<div.*?>.*?<style.*?</style>\s*(.*).*?</div>"
    ...
    def to_markdown(self, include_fence=True):
        ...
        if self.text or self.output:
            ...
            elif "prompt" in self.tags:
                text = "\n".join([s.strip() for s in str(self).splitlines()])
                text = f'<pre style="white-space: pre-wrap; border-left: 10px solid gray; margin-left: 42px;">\n{text}\n</pre>'
            else:
                text = str(self)
...
if __name__ == "__main__":
    ...
    (lesson_root / "episodes" / "files").mkdir(exist_ok=True, parents=True)
    (lesson_root / "learners").mkdir(exist_ok=True, parents=True)
    ...
    for nb_path in sorted(
        ...
        if os.path.basename(nb_path) == "00-template.ipynb":
            continue
        ...
        try:
            ...
            env = os.environ.copy()
            env["PYTHONWARNINGS"] = "ignore:Proactor event loop"
            subprocess.run(
                [
                    "jupyter",
                    "nbconvert",
